...
/**
 * ✅ تنفيذ أداة معينة مع المعاملات (دالة قابلة للاستدعاء)
 * @param {string} toolName - اسم الأداة
 * @param {Object} payload - المعاملات
 * @returns {Promise<Object>} - نتيجة التنفيذ
 */
export async function executeTool(toolName, payload) {
    if (!toolName || !toolsRegistry || !toolsRegistry[toolName]) {
        throw new Error(`الأداة "${toolName}" غير موجودة`);
    }

    const selectedTool = toolsRegistry[toolName];
    ...
    if (typeof selectedTool.handler === 'function') {
        ...
        if (result && result.success === false) {
            throw new Error(result.error || "فشل تنفيذ الأداة");
        }
        
        console.log(`✅ تم تنفيذ الأداة ${toolName} بنجاح`);
        return result;
    }

    throw new Error(`الأداة "${toolName}" ليس لها معالج (handler)`);
}

/**
 * ✅ معالج Express (للاستخدام كـ API endpoint)
 */
