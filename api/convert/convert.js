...
export async function convertFileHandler(req, res) {
  try {
    ...
    if (source.includes('excel') || source.includes('xlsx') || source.includes('xls') || 
        source === 'spreadsheet' || !source) { // إذا ما في source، نفترض Excel
...
<style>
  ...
  th { background-color: #4CAF50; color: white; }
      ...
      // Excel → Image (PNG)
        ...
        // تحويل الجدول إلى صورة باستخدام sharp
    ...
    // ============================================================
      ...
      // قراءة ملف Word (نستخرج النص)
      // ملاحظة: docx لا يدعم قراءة الملفات بسهولة، نستخدم طريقة مبسطة
      
      if (format === 'txt' || format === 'text') {
        // نستخرج النص من المستند (محاكاة)
    ...
    // ============================================================
      ...
      if (format === 'txt' || format === 'text') {
        ...
        // إنشاء Excel جديد من بيانات PDF
        const workbook = new Workbook();
        const worksheet = workbook.addWorksheet('Sheet1');
        worksheet.getCell(1, 1).value = 'مستخرج من PDF';
        worksheet.getCell(1, 2).value = 'عدد الصفحات';
        worksheet.getCell(1, 3).value = pageCount;
        const excelBuffer = await workbook.writeToBuffer();
        resultData = excelBuffer;
